...
Open http://localhost:4321/northstar/.
...
The static site is generated in `docs/`. The configured GitHub Pages URL is https://northstar-enterprise.github.io/northstar/ because this repository name differs from the organization's account name. Update `site` and `base` in `astro.config.mjs` if using a custom domain.
...
- `PUBLIC_JERRY_PHOTO` and `PUBLIC_ELLIOT_PHOTO`: photo URLs. Local images can go in `public/`; include the `/northstar/` base in their URLs.
